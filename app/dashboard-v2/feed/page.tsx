...
export default function DashboardV2Page() {
  ...
  const renderFeedEvent = (event: FeedEvent, index: number) => {
    ...
    if (event.type === 'milestone' && event.data.type === 'milestone') {
      ...
      icon = <ArrowUpDown className="w-4 h-4 text-yellow-400" />;
      ...
      >
        ...
        <div className="flex items-start gap-3 mb-3">
          <Link href={`/dashboard-v2/traders/${event.traderUsername}`} className="group flex-shrink-0">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg group-hover:scale-110 transition-transform" style={getAvatarStyle(event.traderDisplayName)}>
              {event.traderAvatar || event.traderUsername[0].toUpperCase()}
            </div>
          </Link>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <Link href={`/dashboard-v2/traders/${event.traderUsername}`} className="font-semibold text-white hover:text-primary-400 transition-colors truncate">
          ...
          <div className="text-xs text-dark-500 flex-shrink-0">{timeAgo(event.timestamp)}</div>
        </div>

        {/* Description */}
        <p className="text-sm text-dark-400 mb-3 line-clamp-2">{description}</p>
