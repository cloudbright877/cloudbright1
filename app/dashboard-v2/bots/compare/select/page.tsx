  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24 md:pb-6">
      ...
      >
        ...
        <h1 className="text-3xl font-semibold text-gray-900 dark:text-white mb-2">Select Bot</h1>
        <p className="text-gray-600 dark:text-dark-400">Choose a bot to add to comparison</p>
      </motion.div>
      ...
      >
        <div className="relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 dark:text-dark-400" />
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by name, strategy, or risk level..."
            className="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-dark-900/50 border border-gray-200 dark:border-dark-700 rounded-xl text-gray-900 dark:text-white placeholder:text-gray-500 dark:placeholder:text-dark-500 focus:outline-none focus:border-primary-500/50 focus:ring-2 focus:ring-primary-500/20 transition-all"
          />
        </div>
      </motion.div>
        ...
        {filteredBots.map((bot) => {
          ...
          return (
            <motion.button
              ...
              disabled={isSelected}
              className="p-5 rounded-2xl bg-[linear-gradient(135deg,rgba(255,255,255,0.20)_0%,rgba(255,255,255,0.04)_25%,rgba(255,255,255,0.04)_75%,rgba(255,255,255,0.05)_100%)] p-[1.5px] hover:bg-gray-100 dark:hover:bg-dark-800/50 transition-all text-left disabled:opacity-40 disabled:cursor-not-allowed shadow-lg hover:shadow-2xl"
            >
              <div className="bg-gray-50 dark:bg-gradient-to-br dark:from-dark-800/95 dark:to-dark-900/95 rounded-[calc(1rem-1px)] p-5 hover:border-primary-500/50 transition-all">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-gray-600 dark:text-dark-400 truncate">{bot.strategy}</div>
                  <div className="text-xs text-gray-500 dark:text-dark-500 mt-1 line-clamp-1">{bot.description}</div>
                  ...
                  <div className="flex items-center gap-3 mt-2 flex-wrap">
                    ...
                    <span className="flex items-center gap-1 text-[10px] text-gray-600 dark:text-dark-400">
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <div className="text-[10px] text-gray-600 dark:text-dark-400">30d return</div>
                  <div className={`text-sm font-normal mt-1 ${getPercentColor(bot.stats.return1y)}`}>
                    {formatPercent(bot.stats.return1y)}
                  </div>
                  <div className="text-[10px] text-gray-600 dark:text-dark-400">1y return</div>
                </div>
              </div>
              </div>
            </motion.button>
