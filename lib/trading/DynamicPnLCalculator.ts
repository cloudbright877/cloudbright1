...
export class DynamicPnLCalculator {
  ...
  calculatePnLRange(params: CalibrationParams): PnLRange {
    ...
    // Step 2: Base win P&L per trade (without leverage, this would be daily target)
    // With leverage: divide by avgLeverage to account for amplification
    // Formula: baseWinPnL = (dailyTarget / #trades / winRate) / avgLeverage
    const baseWinPnLPercent = (dailyTargetPercent / tradesPerDay / winRate) / avgLeverage;

    // Step 3: Calculate base loss P&L (using risk-reward ratio)
